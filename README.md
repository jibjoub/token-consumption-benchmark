...
literal `calls-to-blogic-getrelatedarticle` mirror, despite that one matching
the question shape more exactly ("who calls this specific entry point"). Two
reasons: (1) `calls-to-blogic-getrelatedarticle` has almost no data (5 `with`,
0 `with-forced`) while `pages-directly-related-to-PrivateMessage-table` has
real volume across all three conditions (22/22/36); (2) CAST Imaging's own
dependency graph doesn't distinguish between "what calls this subprogram" and
...
more false positives than "calls this subprogram" before treating that gap
as meaningful rather than a quirk of one pair of questions.
